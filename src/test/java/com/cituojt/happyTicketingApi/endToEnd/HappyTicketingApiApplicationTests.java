...
public class HappyTicketingApiApplicationTests {
    ...
    @Test
    public void postToTasksReturnsSpecificProjectWithTasks() throws Exception {
        String taskName = "Make DB";
        ...
        mvc.perform(post("/api/v1/projects/" + p.getId() + "/tasks").header("Authorization", this.bearerToken)
                .param("name", taskName)).andDo(print()).andExpect(status().isCreated())
                .andExpect(jsonPath("$.tasks[:1].name", hasItem(taskName)))
                .andExpect(jsonPath("$.tasks[:1].status", hasItem("TO IMPLEMENT")))
                .andExpect(jsonPath("$.tasks[:1].assigned_to", hasItem(u.getEmail())));
  }
}
